...
@test "The build tools should be correctly installed" {
  # JDK 1.7
  run docker run --name ${CONTAINER_NAME} --rm -P -i ${IMAGE_NAME} /opt/java/jdk7/bin/java -version
  assert "java version \"1.7.0_79\"" "${lines[0]}"
  # JDK 1.8
  run docker run --name ${CONTAINER_NAME} --rm -P -i ${IMAGE_NAME} /opt/java/jdk8/bin/java -version
  assert "java version \"1.8.0_91\"" "${lines[0]}"
  # Maven 3.3.3
  run docker run --name ${CONTAINER_NAME} --rm -P -i ${IMAGE_NAME} /opt/maven/maven3/bin/mvn --version
...
@test "The plugins should be installed along with their dependencies" {
  ...
  # is resolved
  run docker run --name ${CONTAINER_NAME} --rm -P -i ${IMAGE_NAME} ls /usr/share/jenkins/ref/plugins/pipeline-stage-view.jpi
  assert "/usr/share/jenkins/ref/plugins/pipeline-stage-view.jpi" "${lines[0]}"
  run docker run --name ${CONTAINER_NAME} --rm -P -i ${IMAGE_NAME} ls /usr/share/jenkins/ref/plugins/workflow-job.jpi
  assert "/usr/share/jenkins/ref/plugins/workflow-job.jpi" "${lines[0]}"
  run docker run --name ${CONTAINER_NAME} --rm -P -i ${IMAGE_NAME} ls /usr/share/jenkins/ref/plugins/workflow-support.jpi
  assert "/usr/share/jenkins/ref/plugins/workflow-support.jpi" "${lines[0]}"
}

@test "The iapf file should be filled properly" {
  run docker run --name ${CONTAINER_NAME} --rm -P -i ${IMAGE_NAME} cat /tmp/iapf
  assert "${JENKINS_ADMIN_USER}:${JENKINS_ADMIN_PASSWORD}" "${lines[0]}"
}

@test "The SETUP_OPTS environment variable should be set properly" {
  run docker run --name ${CONTAINER_NAME} --rm -P -i ${IMAGE_NAME} bash -c "env | grep SETUP_OPTS"
